...
using BitMEXWebsocketTrade.Websocket;
...
namespace BitMEXWebsocketTrade
{
	public partial class Form1 : Form
	{
		...
		List<BitMexWebsocketTrade> trades;
		...
		private void DelegateManagement()
		{
			...
			websocketInterpreter.InterpreterProcessedTrade += UpdateTradesGrid;
		}
		...
		private void btnSubscribeTrades_Click(object sender, EventArgs e)
		{
			string Message = "{\"op\": \"subscribe\", \"args\": [\"trade:XBTUSD\"]}";
			websocketClient.Send(Message);
		}

		//We create the unsubscription string and send to the websocket
		private void btnUnsubscribeTrades_Click(object sender, EventArgs e)
		{
			string Message = "{\"op\": \"unsubscribe\", \"args\": [\"trade:XBTUSD\"]}";
			websocketClient.Send(Message);
		}
		...
		public void UpdateTradesGrid(object source, List<BitMexWebsocketTrade> t)
		{
			int maxTrades = 20;
			foreach(BitMexWebsocketTrade trade in t)
			{
				trades.Add(trade);
			}
			trades = trades.OrderByDescending(tr => tr.Timestamp).Take(maxTrades).ToList();
			dgdTrades.DataSource = trades.Select(tr => new { Side = tr.Side, Price = tr.Price, Size = tr.Size, Time = tr.Timestamp.ToString("HH:mm:ss-fff") }).ToList();
		}
	}
}
